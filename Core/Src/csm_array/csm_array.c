...
/*  Возвращает количество не прочитанных элементов массива
*/
...
//!-
int csm_buff_copy(csm_array *SourceData, csm_array *DstData, uint32_t size)
{
   int valid;
   uint8_t *rd = csm_array_rd_data(SourceData);
   csm_array_reader_jump(SourceData, size);
   valid = csm_array_write_buff(DstData, rd, size);
   return valid;      
}
